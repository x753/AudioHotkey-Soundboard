...
namespace AudioHotkeySoundboard
{
  public partial class MainForm : Form
  {
    ...
    public MainForm()
    {
      ...
      serverAPI = new ServerAPI(Instance);

      if (!serverAPI.PlatformNotSupported)
      {
        serverAPI.StartHttpListener();
      }
    ...
    public void PlaySound_API(int sound)
    {
      if (sound <= soundHotkeys.Count)
      {
        playKeySound(soundHotkeys[sound]);
      }
    }

    public void StopSound_API()
    {
      stopPlayback();
    }

    public List<XMLSettings.SoundHotkey> GetSound_API()
    {
      return soundHotkeys;
    }
    ...
    private void cbFormClosed(object sender, FormClosedEventArgs e)
    {
      ExitApp();
    }
  }
